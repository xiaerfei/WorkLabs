...
- [ ]  PTS 外推的漂移检测（mpv 式，比 OBS 式更完整）
  - 现状：`wl_decoder` 已按 OBS 方式实现（`best_effort_timestamp` 为 NOPTS 时
    用 `next_pts` 外推 + 时长估算），单帧偶尔丢失时真实值出现会自动重新校准，
    风险低
  - 隐患：**连续多帧**丢时间戳时，外推误差逐帧累积（`next_pts` 是滚动叠加），
    真实时间戳重新出现时，前面靠外推撑着的帧可能已经反超后面的真实帧，形成
    非单调时间线
  - 方案（对标 mpv `correct_audio_pts`，`f_decoder_wrapper.c:834-875`）：新真实
    值出现时和"目前靠外推撑着的位置"比较差值——差距在容器舍入误差内（如 1ms）
    忽略，差距较大打警告，差距离谱（如 >5s）判定漂移、重新校准
  - 详见 `Doc/调研/OBS/OBS_mpv_PTS缺失时间戳修复对比.md`
  - 优先级低的原因：M1 仅处理本地完好文件，连续多帧丢时间戳的复合失败场景
    概率很低
