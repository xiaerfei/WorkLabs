...
//  OBSLabs
//
//  从 WorkLabs WLStreamPreview.m 搬入，去掉 WLStreamRenderingProtocol 依赖。
//

#import "WLSourcePreview.h"
#import <stdatomic.h>

static const CGFloat kHandleSize = 10.0;
static const CGFloat kHandleHit  = 12.0;
...
@implementation WLSourcePreview {
...
- (void)enqueuePixelBuffer:(CVPixelBufferRef)pixelBuffer pts:(Float64)pts {
    ...
    CMSampleBufferRef sb = [self createSampleBufferFromPixelBuffer:pixelBuffer pts:pts];
    if (!sb) return;
    [self enqueueSampleBuffer:sb];
    CFRelease(sb);
}
...
#pragma mark - Enqueue (with back-pressure)

- (void)enqueueSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    if (!sampleBuffer) return;
    if (atomic_fetch_add(&_pendingEnqueues, 1) >= 2) {
        atomic_fetch_sub(&_pendingEnqueues, 1);
        return;
    }
    CFRetain(sampleBuffer);
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.videoLayer.sampleBufferRenderer enqueueSampleBuffer:sampleBuffer];
...
- (void)mouseDown:(NSEvent *)event {
    ...
    if (self.selected) {
        ...
        if ([self.delegate respondsToSelector:@selector(sourcePreviewDidRequestSelect:)]) {
            [self.delegate sourcePreviewDidRequestSelect:self];
        }
    }
...
- (void)rightMouseDown:(NSEvent *)event {
    ...
    if (!self.selected &&
        [self.delegate respondsToSelector:@selector(sourcePreviewDidRequestSelect:)]) {
        [self.delegate sourcePreviewDidRequestSelect:self];
    }
...
- (void)emitZOrderAction:(WLZOrderAction)action {
    if ([self.delegate respondsToSelector:@selector(sourcePreview:didRequestZOrderAction:)]) {
        [self.delegate sourcePreview:self didRequestZOrderAction:action];
    }
}

- (void)deselect:(id)sender {
    if ([self.delegate respondsToSelector:@selector(sourcePreviewDidRequestDeselect:)]) {
        [self.delegate sourcePreviewDidRequestDeselect:self];
    }
}

- (void)removeSelf:(id)sender {
    if ([self.delegate respondsToSelector:@selector(sourcePreviewDidRequestRemove:)]) {
        [self.delegate sourcePreviewDidRequestRemove:self];
    }
}
