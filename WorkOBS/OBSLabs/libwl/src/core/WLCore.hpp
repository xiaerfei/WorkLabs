...
public:
    ...
    static WLSource *AddSource(const char *type_id, const char *settings);

    // 摘除即销毁：出表（与 tick 遍历互斥）后 delete。
    static void RemoveSource(WLSource *src);

    // 锁内遍历所有源（graphics tick 用）。回调内勿 add/remove —— 同一把锁，会死锁。
    static void ForeachSource(void (*fn)(WLSource *src, void *ctx), void *ctx);

    // 注册 per-source 帧输出回调，转发给内部 graphics（对齐 OBS 走全局 API，不直接暴露内部）。
    // 须在 Startup 之后调用；未 Startup 时忽略。cb=NULL 注销。
    static void SetFrameOutput(wl_frame_output_cb cb, void *ctx);
};
