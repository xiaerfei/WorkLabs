...
static void PrintAvError(const char *prefix, int errnum) {
...
int WLDecoder::FfmpegInit() {
    AVFormatContext *ctx = NULL;
    int ret = avformat_open_input(&ctx, path_, NULL, NULL);
    if (ret < 0) {
        PrintAvError("avformat_open_input", ret);
        return -1;
    }
    fmt_ctx_ = ctx;
    return 0;
...
int WLDecoder::FindStream() {
    int ret = avformat_find_stream_info(fmt_ctx_, NULL);
    if (ret < 0) {
        PrintAvError("avformat_find_stream_info", ret);
        return -1;
    }
    return 0;
}

int WLDecoder::FindVideoStream() {
    const AVCodec *codec = NULL;
    int streamIndex = av_find_best_stream(fmt_ctx_, AVMEDIA_TYPE_VIDEO, -1, -1, &codec, 0);
    if (streamIndex < 0) {
        PrintAvError("avformat_find_video_stream_info", streamIndex);
        return -1;
    }
    video_index_ = streamIndex;
    return 0;
}

// 配置 video decoder，失败返回 -1，成功返回 0
// 注意：失败时不自行释放，统一由 dtor 收尾
int WLDecoder::CfgdVideo() {
    AVStream *stream = fmt_ctx_->streams[video_index_];
    // 1. 查找对应的基础解码器
    const AVCodec *codec = avcodec_find_decoder(stream->codecpar->codec_id);
    if (!codec) {
        PrintAvError("Video: can not find codec", AVERROR(ENOMEM));
        return -1;
    }
    ...
    if (!video_codec_ctx_) {
        PrintAvError("Video: video_codec_ctx_ is null", AVERROR(ENOMEM));
        return -1;
    }
    ...
    if (ret < 0) {
        PrintAvError("Video: avcodec_parameters_to_context failed", ret);
        return -1;
    }
    ...
    if (hw_type_) {
        // 局部叫 device_type：C 版局部变量与成员同名 hw_type 会遮蔽；现在成员带
        // 尾缀已无冲突，保留这个名字纯粹因为它更达意
        enum AVHWDeviceType device_type = av_hwdevice_find_type_by_name(hw_type_);
        if (device_type == AV_HWDEVICE_TYPE_NONE) {
            fprintf(stderr, "Video: unknown hw device type '%s', falling back to software\n", hw_type_);
            video_codec_ctx_->thread_count = 0;
            goto open_codec;
        }
        int hw_err = av_hwdevice_ctx_create(&hw_device_ctx_, device_type, NULL, NULL, 0);
        if (hw_err >= 0) {
            // 创建成功，将其引用绑定到解码器上下文中
            AVBufferRef *ref = av_buffer_ref(hw_device_ctx_);
            if (ref) {
                video_codec_ctx_->hw_device_ctx = ref;
            } else {
                // av_buffer_ref OOM，降级为软解
                PrintAvError("Video: av_buffer_ref failed, falling back to software", AVERROR(ENOMEM));
            ...
            PrintAvError("Video: hw codec error", hw_err);
            hw_device_ctx_ = NULL;
            /*
             多线程安全（可选优化）：
...
open_codec:
    // 5. 正式打开解码器
    ret = avcodec_open2(video_codec_ctx_, codec, NULL);
    if (ret < 0) {
        PrintAvError("Video: avcodec_open2 failed", ret);
        return -1;
    }
    return 0;
}

int WLDecoder::FindAudioStream() {
    const AVCodec *codec = NULL;
    int streamIndex = av_find_best_stream(fmt_ctx_, AVMEDIA_TYPE_AUDIO, -1, -1, &codec, 0);
    if (streamIndex < 0) {
        PrintAvError("avformat_find_audio_stream_info", streamIndex);
        return -1;
    }
    audio_index_ = streamIndex;
    return 0;
}

// 配置 audio decoder，失败返回 -1，成功返回 0
// 注意：失败时不自行释放，统一由 dtor 收尾
int WLDecoder::CfgdAudio() {
    AVStream *stream = fmt_ctx_->streams[audio_index_];

    // 1. 查找对应的音频解码器
    const AVCodec *codec = avcodec_find_decoder(stream->codecpar->codec_id);
    if (!codec) {
        PrintAvError("Audio: can not find codec", AVERROR(ENOMEM));
        return -1;
    }
    ...
    if (!audio_codec_ctx_) {
        PrintAvError("Audio: audio_codec_ctx_ is null", AVERROR(ENOMEM));
        return -1;
    }
    ...
    if (ret < 0) {
        PrintAvError("Audio: avcodec_parameters_to_context failed", ret);
        return -1;
    }

    // 4. 打开解码器
    ret = avcodec_open2(audio_codec_ctx_, codec, NULL);
    if (ret < 0) {
        PrintAvError("Audio: avcodec_open2 failed", ret);
        return -1;
    }
...
WLDecoder::WLDecoder(const char *path, const char *hw_type) {
    ...
    hw_type_ = hw_type ? strdup(hw_type) : NULL;   // NULL = 纯软解，原样保留
    fmt_ctx_         = NULL;
    video_index_     = -1;   // C 版 calloc 给 0；-1 更诚实（0 是合法流号），receive 侧本就按 >=0 判断
    audio_index_     = -1;
    video_codec_ctx_ = NULL;
    audio_codec_ctx_ = NULL;
    hw_device_ctx_   = NULL;
    ...
    video_next_pts_ns_      = AV_NOPTS_VALUE;
    video_last_pts_ns_      = AV_NOPTS_VALUE;
    video_last_duration_ns_ = 0;
    audio_next_pts_ns_      = AV_NOPTS_VALUE;

    // 分步初始化：C 版每步失败 free+return NULL；这里失败直接 return（ok_ 留 false），
    // 调用方查 Valid() 后 delete，清理统一走 dtor
    if (FfmpegInit()      != 0) return;
    if (FindStream()      != 0) return;
    if (FindVideoStream() != 0) return;
    if (CfgdVideo()       != 0) return;
    if (FindAudioStream() != 0) return;
    if (CfgdAudio()       != 0) return;

    pkt_         = av_packet_alloc();
    video_frame_ = av_frame_alloc();
    audio_frame_ = av_frame_alloc();
    if (!pkt_ || !video_frame_ || !audio_frame_) return;
...
HWAccelList WLDecoder::GetSupportedHwaccels() {
    HWAccelList list;
    list.count = 0;
...
wl_read_result_t WLDecoder::Read() {
    if (eof_reached_) return WL_READ_EOF;

    int ret = av_read_frame(fmt_ctx_, pkt_);   // pkt_ 复用同一个 packet，避免每次堆分配
    if (ret == AVERROR_EOF) {
        // 文件读完，给两个 codec 送 NULL 触发 drain（EOF 排空，非 seek flush）。
        // 出错/EOF 时 av_read_frame 已把 pkt_ 置空，无需 unref。
        // drain send 返回值：EOF / EAGAIN 都是预期的软状态（重复 flush / 尚有
        // 输出待取）→ 忽略；只有硬错误才记日志，且不改流程——后续 receive
        // 会自然走到 EOF 结束。
        eof_reached_ = true;
        int vret = video_codec_ctx_ ? avcodec_send_packet(video_codec_ctx_, NULL) : 0;
        int aret = audio_codec_ctx_ ? avcodec_send_packet(audio_codec_ctx_, NULL) : 0;
        if (vret < 0 && vret != AVERROR_EOF && vret != AVERROR(EAGAIN))
            PrintAvError("avcodec_send_packet(video drain)", vret);
        if (aret < 0 && aret != AVERROR_EOF && aret != AVERROR(EAGAIN))
            PrintAvError("avcodec_send_packet(audio drain)", aret);
        return WL_READ_EOF;
    }
    if (ret < 0) {
        PrintAvError("av_read_frame", ret);
        return WL_READ_ERROR;
    }

    int send_ret = 0;
    wl_read_result_t result;
    if (pkt_->stream_index == video_index_ && video_codec_ctx_) {
        send_ret = avcodec_send_packet(video_codec_ctx_, pkt_);
        result = WL_READ_VIDEO;
    } else if (pkt_->stream_index == audio_index_ && audio_codec_ctx_) {
        send_ret = avcodec_send_packet(audio_codec_ctx_, pkt_);
        result = WL_READ_AUDIO;
    } else {
        result = WL_READ_SKIP;   // 字幕 / 数据流，跳过
    }

    av_packet_unref(pkt_);   // 释放本次引用，packet 结构留给下次复用

    // 串行 drain-first 设计下 send 不该返回 EAGAIN；真错误（codec 异常等）上报。
    if (send_ret < 0) {
        PrintAvError("avcodec_send_packet", send_ret);
        return WL_READ_ERROR;
    }
    return result;
}
...
static wl_frame_result_t ReceiveFrame(AVCodecContext *ctx,
                                      AVFrame *frame,
...
wl_frame_result_t WLDecoder::ReceiveVideo(AVFrame **out_frame, int64_t *out_pts_ns) {
    if (!video_codec_ctx_) {
        video_drained_ = true;   // 无视频流 → 视为永久排空
        return WL_FRAME_EOF;
    }

    AVRational tb = (video_index_ >= 0)
        ? fmt_ctx_->streams[video_index_]->time_base
        : (AVRational){1, 1};

    // video_frame_ 复用：avcodec_receive_frame 内部会先对它做 av_frame_unref，
    // 成功时内容被 move 进新分配的 out_frame（见 ReceiveFrame），
    // frame 结构体本身留给下次调用复用。
    wl_frame_result_t r = ReceiveFrame(video_codec_ctx_, video_frame_, tb,
    ...
    int64_t last_pts_ns = video_last_pts_ns_;
    int64_t pts_ns = *out_pts_ns;
    if (pts_ns == AV_NOPTS_VALUE)
        pts_ns = video_next_pts_ns_;
    ...
    } else if (video_last_duration_ns_ > 0) {
        /// 第三道防线：用前一帧的“历史经验”
        /// 如果这是前几帧，连 last_pts_ns 都还没有呢？那就看看更早之前成功估算出的历史时长
        /// video_last_duration_ns_。既然上一次是这么久，在没有新数据的情况下，假设视频是
        /// 恒定帧率（CFR），继续沿用这个经验值。
        duration_ns = video_last_duration_ns_;
    } else {
        /// 第四道防线：终极无脑兜底（最后一道防线）
    ...
    video_last_duration_ns_ = duration_ns;
    video_last_pts_ns_ = pts_ns;
    video_next_pts_ns_ = pts_ns + duration_ns;
    *out_pts_ns = pts_ns;

    return WL_FRAME_OK;
}

wl_frame_result_t WLDecoder::ReceiveAudio(AVFrame **out_frame, int64_t *out_pts_ns) {
    if (!audio_codec_ctx_) {
        audio_drained_ = true;   // 无音频流 → 视为永久排空
        return WL_FRAME_EOF;
    }

    AVRational tb = (audio_index_ >= 0)
        ? fmt_ctx_->streams[audio_index_]->time_base
        : (AVRational){1, 1};

    // audio_frame_ 复用，语义同 ReceiveVideo。
    wl_frame_result_t r = ReceiveFrame(audio_codec_ctx_, audio_frame_, tb,
                                       out_frame, out_pts_ns);
    if (r == WL_FRAME_EOF) {   // 仅真 EOF 才算排空；EAGAIN 不是
        audio_drained_ = true;
        return r;
...
bool WLDecoder::Drained() const {
    return video_drained_ && audio_drained_;
}
