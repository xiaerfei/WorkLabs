...
//  create 半途失败 → ctor 提前 return（ok 留 false），调用方查 valid()
...
class WLDecoder {
    ...
    char *path;      // strdup 拥有（C 版存裸指针、靠调用方保证生命周期，这里改为拥有）
    char *hw_type;   // strdup 拥有；NULL = 纯软解

    AVFormatContext *fmt_ctx;
    int video_index;  ///< 视频流索引；-1 = 尚未找到
    int audio_index;  ///< 音频流索引；-1 = 尚未找到

    AVCodecContext *video_codec_ctx; ///< Video 解码器
    AVCodecContext *audio_codec_ctx; ///< Audio 解码器
    AVBufferRef    *hw_device_ctx;   ///< 硬解码设备上下文
    ...
    bool ok;            ///< ctor 全链路是否成功（构造函数没有返回值，靠它表达失败）
    ...
    int  ffmpeg_init();        // avformat_open_input
    int  find_stream();        // avformat_find_stream_info
    int  find_video_stream();  // av_find_best_stream(VIDEO)
    int  cfgd_video();         // 配置 video decoder（含硬解，失败降级软解）
    int  find_audio_stream();  // av_find_best_stream(AUDIO)
    int  cfgd_audio();         // 配置 audio decoder
    void video_codec_free();   // codec ctx + hw device ctx
    void audio_codec_free();
...
public:
    ...
    bool valid() const { return ok; }   // 创建是否成功；false 时唯一合法操作是 delete
     ...
     */
    wl_frame_result_t receive_video(AVFrame **out_frame, int64_t *out_pts_ns);

    /**
     * 尝试从 audio_codec_ctx 接收一帧（非阻塞）。
     * *out_frame 成功时由调用方 av_frame_free。
     */
    wl_frame_result_t receive_audio(AVFrame **out_frame, int64_t *out_pts_ns);
     ...
     */
    static HWAccelList get_supported_hwaccels();
};
