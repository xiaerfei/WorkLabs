...
> **关联**：`Doc/调研/mpv/MPV_调研与预览CPU优化报告.md`（§2 粗略碰过线程/队列；本文深挖时间戳与队列）。承接「时间戳问题」讨论与「优化② 渲染线程精确等待」待办。
> **配套**：`Doc/调研/mpv/MPV_时间戳源码深挖.md` —— 本文偏「取经清单 + 队列/同步全景」，配套那篇偏**源码级逐函数**追时间戳生命周期（产生/异常/同步/重置），要看具体函数体与边界条件看那篇。
