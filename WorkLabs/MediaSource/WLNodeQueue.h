...
/// 队列存储的数据类型（视频/音频）
@property (nonatomic, assign, readonly) WLDecodeType type;
...
@property (nonatomic, strong) WLDecodeNode *head;
@property (nonatomic, strong) WLDecodeNode *tail;
 ...
 */
- (nullable WLDecodeNode *)deQueueWithBlock:(BOOL)block;

/**
 查看队头节点（不移除）
 */
- (nullable WLDecodeNode *)peek;
