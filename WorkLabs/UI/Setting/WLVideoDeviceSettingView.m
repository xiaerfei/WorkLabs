...
@interface WLVideoDeviceSettingView () <NSComboBoxDataSource, NSComboBoxDelegate>
...
- (void)defaultButtonClick {
    [self resetToDefault];
    if (self.delegate && [self.delegate respondsToSelector:@selector(videoDeviceSettingViewDidClickDefault:)]) {
        [self.delegate videoDeviceSettingViewDidClickDefault:self];
    }
}
...
- (void)confirmButtonClick {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(videoDeviceSettingView:didClickConfirmWithDevice:preset:useBuffer:)]) {
